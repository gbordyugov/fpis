...
/*
 * this trait is meant to be inherited by objects
 * hence its methods always accept the actual instance as first
 * argument
 */
...
trait Monadic[F[_],A] {
  /*
   * this is the object that holds the monadic operations
   */
  val F: Monad[F]

  /*
   * we make those operations available here by importing
   */
  import F._

  /*
   * that's the actual monadic value
   */
  def get: F[A]
  private val a = get
  ...
  def map2[B,C](b: F[B])(f: (A,B) => C): F[C] =
    F.map2(a,b)(f)
